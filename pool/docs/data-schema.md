...
- **2026-05-01: orphan-entity confusion (resolved 2026-05-02).** Between the v1.8.0 migration and 2026-05-01, the logger and `sensor.pool_automation_status` template were pointing at `valve.omnilogic_pool_waterfall` while the blueprint operated on `valve.omnilogic_pool_waterfall_2`. We initially "fixed" the divergence on 2026-05-01 by pointing the logger at `valve.omnilogic_pool_waterfall_2`, on the assumption that `_2` was the live entity. **That was wrong-direction** — verified via Developer Tools → States on 2026-05-02 that `valve.omnilogic_pool_waterfall` (no suffix) is the live entity (state `open`, full attributes including `omni_function: Waterfall`). `_2` is/became the orphan, returning `unavailable`. Reverted on 2026-05-02 — all references (templates, configuration, blueprint input via automations.yaml, `state_logger.py`) now point at `valve.omnilogic_pool_waterfall`. The 2026-04-29→2026-05-02 CSV waterfall_state rows are still likely useless (they were tracking an entity that may have been reporting stale state), but the live blueprint is now actually controlling the right valve.
