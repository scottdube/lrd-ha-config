...
### Pool automation
- **Blueprint version:** v1.10.1 (deployed 2026-05-02); **v1.10.2 patched in repo 2026-05-02, awaiting push + deploy** — integration-recovery debounce per ADR-016. Adds `input_boolean.pool_integration_recovering`, a watcher automation that flips it on for 5 min after every OmniLogic Local recovery, and a gate condition on both detection automations in `packages/pool/pool_modes.yaml`. Closes a class of false-positive lockout engagements where the controller's autonomous state reconciliation during a HA restart looked like panel-toggle events to detection. Validated cause via 2026-05-02 10:25–10:40 incident: HA restart for v1.10.1 deploy → integration disconnect → controller cycled pump off+waterfall closed+heater on autonomously → false-positive lockout engaged + cleared, blueprint paused for ~3 min. **v1.10.1** (current deployed) — single-line `current_water_temp` fallback fix per ADR-013. Closes the bug where the PUMP START gate fired at every poll after a pump-off event because `float(75)` made `hours_to_heat=14` whenever the OmniLogic water sensor read `unknown`. Marginal cost of v1.10.1's bug: ~$150–250/year overnight pump waste.
...
- **Thermal/condensation analysis added to ADR-015 2026-05-02.** Empirical observation during the calibration session — visible condensation formed inside the case within 15 min of submerging the lower body in 32°F ice bath, without solar gain or gasket leakage. Root cause: trapped humid air condenses out whenever interior surfaces drop below dew point, which happens daily in pool service from overnight cooling + solar gain on the clear dome. Promoted moisture-mitigation items from optional to required: hydrophobic vent membrane (top priority — addresses pressure-cycle pumping at root), indicating silica gel desiccant, conformal coating, dielectric grease on contacts, reflective dome treatment + LCD removal, Viton gasket. Estimated BOM ~$80–110, dominated by moisture mitigation (~85% of spend). Full BOM in `pool/docs/external-water-temp-bom.md`.
...
- **Pool maintenance day handling** (raised 2026-05-02, scoped 2026-05-02).
  ...
  - **Current mitigation (workable interim)**: tech turns off breaker at pool control box. v1.9.0's integration-health gate (`states(pump_switch) != 'unavailable'`) skips the MAIN POLL when integration is unreachable, so HA respects physical lockout-tagout automatically.
  - **Residual risk**: when breaker comes back on, next /10 poll will evaluate fresh — could fire PUMP START within 10 min of restoration. Acceptable only if tech protocol is "breaker stays off until everything is reassembled."
  - **Breaker wear concern (raised by pool tech 2026-05-02)**: weekly breaker cycling on the 60A pool circuit accelerates contact wear under motor-start inrush. Bumps priority on ADR-011 (door switch + soft lockout) — without it, every routine service requires a breaker cycle that shortens breaker life. Door switch + soft lockout shifts routine pauses to non-cycling mechanisms, reserving the breaker for actual disassembly LOTO (its intended use).
  - **Future fix (ADR-011 candidate)**: combined "service active" gate composed of FOUR independent signals — any one true → blueprint pauses:
    1. **External state-change detection** (raised 2026-05-02). Tech toggles pump off via Hayward panel → HA observes state change with no HA user_id or parent_id (integration-coordinator-originated change). Sets `input_boolean.pool_externally_controlled`. **Highest leverage** — no hardware, no tech-workflow change, captures the actual signal she already creates. Cross-validate with `omni_filter_state` (`WAITING_TURN_OFF_MANUAL`) and `omni_why_on` enums (FilterState/FilterWhyOn library values exposed as attributes; `local_filter_state_enum` column in logger v2 will provide empirical validation).
    2. **Door switch on pool control box**. Z-Wave LR (Zooz ZSE41 800LR, ~$25) preferred — Scott has Zooz ZST39 LR controller; 908 MHz penetrates cement far better than 2.4 GHz which Hayward WiFi struggled with at this location. Captures cases where door is opened but pump isn't toggled (visual inspection, sensor swap, etc.). ESP32+Ethernet on the existing pool-side ethernet run as fallback if RF still flaky.
    3. **`input_boolean.pool_service_lockout`** soft toggle for quick tasks. Auto-clear at midnight so forgotten lockouts don't disable the pool.
    4. **Pump entity unavailable** (already handled by v1.9.0 integration-health gate; respects breaker LOTO — but we want to AVOID using breaker for routine service per tech's wear concern).
  - **Reset logic for signals 1, 2, 3**:
    - Symmetric for signal 1: external pump ON detected (no HA context) → clear lockout immediately. Tech's panel-toggle workflow is fully bidirectional, no Scott action needed.
    - Symmetric for signal 2 (door switch): door close (debounced 5+ min to filter bumps) → clear lockout.
    - Manual "Resume" via mobile notify action → clear immediately.
    - Auto-clear at midnight as dead-man backstop.
  - **Surgical scope** (raised 2026-05-02): `service_active` gates ONLY equipment-control branches (PUMP START, WATERFALL ON/END, NOT-A-SWIM-DAY pump-off, HEATER+SPEED). It does NOT gate POOL LIGHT branches or MIDNIGHT RESET. Pool light still comes on at dark even during service work — light has nothing to do with equipment being serviced.
  - Sensor unavailability handling: **fail-safe** (treat `unavailable` as `open` / locked-out) + mobile notify so dead batteries get fixed.
  - May merge with ADR-007 (manual override) into a single "blueprint pause/resume" ADR if patterns overlap enough during design.
