...
**Last updated:** 2026-05-02 (v1.10.1 + v1.10.2 deployed and validated; logger v2 phase 1.5 patched in repo — state-change triggers + context columns, awaiting push + deploy; ADR-015 accepted — external water temp sensor v1 path = case-reuse + NTC-reuse, EOM 2026-05-31 deploy target; ADR-015 NTC calibration + thermal-condensation analysis + BOM committed; **ADR-015 firmware bench validation complete on actual XIAO ESP32-C6 hardware — ±0.5°F vs reference probe, deep sleep cycle running, schematic generated, WiFi/OTA gotchas documented**; Mac mini rsync still queued)
