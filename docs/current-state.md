...
- **Custom battery holder (3-piece printed assembly).** Purchased AA holder won't fit case interior (case ID narrows below upper zone). Designed Battery_Cap (32.5 mm OD), ESP32_Deck (32.5 mm OD), Battery_Base (29.5 mm OD) in Fusion 360. **v6 base finalized 2026-05-03** — lips re-clocked from (0, ±7.5) Y-axis to (±7.25, 0) X-axis matching the existing contact slots; lips are partial annular arcs (15/16 mm ID/OD × 1 mm tall) on the upper/lower-outboard quadrants of each cell, open inboard so cells locate against each other and no X-intersection ridge interferes with seating; outboard portions clipped at disc OD so nothing exceeds 29.5 mm. v3 cap previously confirmed fits 33 mm upper zone. Iteration history + open items in `pool/docs/external-water-temp-battery-holder.md`. STL: `Battery_Base_v6.stl`. **Original purchased contacts are non-salvageable (peened during assembly).** Next contact source: 29×12 mm AA spring/plate replacement sets (uxcell, SQXBK, etc.) — see same doc for sourcing options. Open decision: surface-mount contacts (delete contact slots, simplest CAD) vs recessed pockets (29×13 mm × 1 mm deep, cleaner mechanical seat).
- **Open for next session:**
  - Print Battery_Base_v6 + insert into case to confirm it now reaches design depth (the v3 case-fit issue should be resolved)
